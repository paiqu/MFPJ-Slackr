...
def test_set_name(register_and_login_user_1):
    '''
    This function for test whether can set name successfully
    '''
    ...
    assert payload['users'][0]['name_first'] == 'Zane'
    assert payload['users'][0]['name_last'] == 'James'
def test_error_no_type(register_and_login_user_1):
    '''
    This function is for test raising error if type nothing
    '''
    user_1_token = 'b\'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1X2lkIjoiMSJ9.N0asY15U0QBAYTAzxGAvdkuWG6CyqzsR_rvNQtWBmLg\''
    response = register_and_login_user_1
    assert response['u_id'] == 1
